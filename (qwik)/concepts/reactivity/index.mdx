...
1. Usando el registro explícito de escuchadores mediante `.subscribe()` (por ejemplo, RxJS).
...
4. En el navegador, el usuario hace clic en el botón. Debido a que el controlador de eventos de clic hace referencia a `store`, Qwik restaura el proxy del store. El proxy contiene el estado de la aplicación (el contador) y la suscripción que asocia a `Counter` con `state.count`.
...
2. Si el usuario hace clic en `ocultar`, `ComplexCounter` se vuelve a renderizar. En el nuevo renderizado, se borran todas las suscripciones y se registran nuevas. En esta ocasión, el JSX no lee `store.count`. Por lo tanto, solo `store.visible` se agrega a la lista de suscripciones.
3. Si el usuario hace clic en `incrementar`, se actualiza `store.count`, pero esto no provoca que el componente se vuelva a renderizar. Esto es correcto porque el contador no es visible, por lo que volver a renderizar sería una operación sin efectos.
4. Si el usuario hace clic en `mostrar`, el componente se volverá a renderizar y esta vez el JSX leerá tanto `store.visible` como `store.count`. La lista de suscripciones se actualiza una vez más.
5. Ahora, al hacer clic en `incrementar`, se actualiza `store.count`. Como el contador es visible, `ComplexCounter` está suscrito a `store.count`.
...
En los ejemplos anteriores, Qwik envolverá automáticamente los objetos secundarios `person` y `location` en un proxy y creará correctamente suscripciones en todas las propiedades anidadas.
...
- Cada vez que se hace clic en un botón, se incrementa uno de los tres contadores. Un cambio en el estado del contador provocará que el componente `MyApp` se vuelva a renderizar en cada clic.
