...
Para que una aplicación web sea interactiva, es necesario tener una forma de responder a los eventos del usuario. Esto se logra registrando funciones callback (de devolución de llamada) en la plantilla JSX. Los controladores de eventos se registran utilizando el atributo `on{NombreEvento}$`. Por ejemplo, el atributo `onClick$` se utiliza para escuchar los eventos de clic (`click`).

```tsx /onClick$/
<button onClick$={() => alert('¡CLIC!')}>Haz clic aquí</button>
```
...
Observa que `onClick$` termina con el sufijo [`$`](/docs/(qwik)/advanced/dollar/index.mdx). Esto es una indicación tanto para el [Optimizador](/docs/(qwik)/advanced/optimizer/index.mdx) como para el desarrollador de que ocurre una transformación especial en esta ubicación. La presencia del sufijo `$` implica un límite de lazy-load (carga lenta) aquí. El código asociado al handler (controlador) del `click` no se cargará en la VM (máquina virtual) de JavaScript hasta que el usuario active el evento `click`. Sin embargo, se cargará [en la caché del navegador](/docs/advanced/speculative-module-fetching/) precipitadamente (eagerly loaded) para no causar retrasos en las primeras interacciones.

> En aplicaciones del mundo real, el listener puede referirse a código complejo. Al crear un límite de lazy-load (con `$`), Qwik puede eliminar todo el código detrás del listener del clic y retrasar su carga hasta que el usuario haga clic en el botón.
...
> **Nota:** Si extraes el handler de eventos, debes envolver manualmente el handler de eventos en `$(...handler...)` para que se adjunte de forma diferida.
...
export default component$(() => {
  const count = useSignal(0);
  const print = $((ev) => console.log('¡CLIC!', ev));
  const increment = $(() => count.value++);

  // Cuando se hace clic en el botón, se imprimirá "¡CLIC!" en la consola, se incrementará el contador y se enviará un evento a Google Analytics.
  return (
    <button
...
El primer argumento del handler de eventos es el objeto `Event`. Este objeto contiene información sobre el evento que activó el handler. Por ejemplo, el objeto `Event` para un evento de `clic` contiene información sobre la posición del mouse y el elemento que se ha hecho clic. Puedes consultar la [documentación de MDN](https://developer.mozilla.org/es/docs/Web/API/Event) para obtener más detalles sobre cada evento del DOM.
...
export default component$(() => {
  ...
  return (
    ...
    >
      <p>
        Clic en: ({position.value?.x}, {position.value?.y})
      </p>
    </div>
  );
});
...
Debido a la naturaleza asíncrona de Qwik, la ejecución del handler de un evento puede retrasarse porque la implementación aún no se ha cargado en la VM de JavaScript. Debido a la naturaleza asíncrona del procesamiento de eventos en Qwik, las siguientes APIs en un objeto `Event` no funcionarán:
...
export default component$(() => {
  ...
  return (
    <section onClick$={(event, currentTarget) => {
      ...
      Haz clic en cualquier texto <code>target</code> y <code>currentElm</code> del evento.
      <hr/>
      <p>¡Hola <b>Mundo</b>!</p>
      <hr/>
      <ul>
...
```tsx
<CmpButton onClick={() => alert('¡CLIC!')}>¡Haz clic en mí!</CmpButton>
```
...
export default component$(() => {
  return <CmpButton onClick$={() => alert('¡CLIC!')}>¡Haz clic en mí!</CmpButton>;
});
...
El hook `useOn[|window|document]()` agregará un escuchador de eventos basado en el DOM a nivel de componente de forma programática. Esto es útil cuando deseas crear tus propios use-hooks o si no conoces el nombre del evento al momento de la compilación.
