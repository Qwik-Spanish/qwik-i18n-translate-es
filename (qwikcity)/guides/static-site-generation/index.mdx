...
La función `onStaticGenerate` debe ser exportada desde el nivel superior del módulo y debe devolver un objeto con una propiedad `params`. La propiedad `params` debe ser un array de objetos, donde cada objeto es un conjunto de parámetros para la ruta. Por ejemplo, si la ruta es `/producto/:id`, entonces el array `params` debe ser un array de objetos con una propiedad `id`.
